...
export const RepportsStats = (props: Props) => {
    const repports = props.type === StatsType.All ? props.repports : getMatchingRepports(props);
...
const getNumberPublicationPlacements = (repports: Repport[]) => {
    return repports.length > 0 ? repports.map((repport: Repport) => repport.publications).reduce((previous, current) => previous + current) : 0;
}

const getNumberOfVideoShowings = (repports: Repport[]) => {
    return repports.length > 0 ?  repports.map((repport: Repport) => repport.videos).reduce((previous, current) => previous + current) : 0;
}

const getNumberOfHours = (repports: Repport[]) => {
    return repports.length > 0 ?  repports.map((repport: Repport) => repport.hours).reduce((previous, current) => previous + current) : 0;
}

const getNumberOfReturnVisits = (repports: Repport[]) => {
    return repports.length > 0 ?  repports.map((repport: Repport) => repport.visits).reduce((previous, current) => previous + current) : 0;
}

const getNumberOfStudies = (repports: Repport[]) => {
    return repports.length > 0 ?  repports.map((repport: Repport) => repport.courses).reduce((previous, current) => previous + current) : 0;
}

const getMatchingRepports = (props: Props): Repport[] => {
    return props.repports.filter((repport: Repport) => {
        const publisher = props.publishers.find(p => p.id === repport.publisherId);
