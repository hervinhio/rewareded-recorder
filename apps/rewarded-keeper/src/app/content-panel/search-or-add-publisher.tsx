...
export function SearchAndAddPublisher(props: Props) {
  ...
  if (!Users.getCurrent().admin) return null;

  return (
    <Popup
      placement="bottom-start"
      content={() => (
        <SearchAndAddPublishercontents onAdd={props.onAdd} value={value} />
      )}
      isOpen={!!value}
      autoFocus={false}
      shouldFlip={true}
...
function SearchAndAddPublishercontents({
  onAdd,
  value,
}: {
  value: string;
  ...
  const nonMatchedPublishers = publishers.filter(
    (p) =>
      p.groupId !== groupId &&
      getPublisherName(p)
        .toLocaleLowerCase()
