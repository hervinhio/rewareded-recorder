...
export const RepportsView = (props: Props) => {
    ...
    const [ repportUnderEdit, setRepportUnderEdit ] = useState<Repport | undefined>();

    useEffect(() => {
        Repports.byPublisherId(props.publisher.id)
            .then((repps) => setRepports(repps.sort(sortRepportsByMonth)), (err) => console.error(err));
    }, [props.publisher.id]);
    ...
    return (
        <>
            ...
            {showRepportModal && <RepportModal repport={repportUnderEdit} onHide={() => {
                setShowRepportModal(false);
                setRepportUnderEdit(undefined);
            }} publisherId={props.publisher.id} show={showRepportModal}/>}
        </>
    );
};
